...
export class Seeder extends Construct {
  constructor(scope: Construct, id: string, props: Props) {
    ...
    parameters.RequestItems[props.tableName] = this.convertToDynamoJson(
      props.json
    );
  ...
  protected convertToDynamoJson(data: any) {
    if (Array.isArray("array")) return this.marshall(data);
    if (typeof data == "object") return this.marshall([data]);
    throw new Error(
      "supplied data must be a JSON object or an array of JSON objects"
    );
  }
